...
namespace EMAI.LND
{
    public class GastosDiaOperaciones : IGastosDiaOperaciones
    {
        ...
        //Actualizar GastoDia
        public async Task<bool> ActualizarGastosDia(int IdGastoDia, DateTime Fecha, string NoPedido, string Proveedor, string Descripcion,
            decimal Cantidad, decimal Subtotal, decimal Total)
        {
            using var db = AppRepositoryFactory.GetAppRepository();
            var rsp = await db.ActualizarGastosDia(IdGastoDia,Fecha,NoPedido, Proveedor, Descripcion,
